...
This means the policy depends on managed `aws_security_group` resources and then checks whether each of those groups is referenced by an ENI attachment path. Default security groups are out of scope for this periodic rule and are not evaluated here.
...
trace:
      ...
      ec2-security-group-attached-to-eni-periodic.sentinel:1:1 - Rule "main"
        Value:
          true
```
...
- `aws_default_security_group` resources are not evaluated because the AWS Config periodic rule applies to non-default security groups.